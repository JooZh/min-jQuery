(function(window){
	...
	// 遍历节点数组
	E.fn.each = function(array, callback){
	...
	// 通用绑定事件
	E.fn.on = function(event, target, callback){
	...
	// 移除事件
	E.fn.off = function(event, callbackName){
		this.each(item => {
	...
	// 遍历节点数组
	E.fn.createElement = function(htmlStr){
	...
	// 在选中元素的头部插入节点
	E.fn.appendBefore = function(val){
		return this.appendWhere(val, 0);
	}
	// 在选中元素的何处插入节点
	E.fn.appendWhere = function(val,num){
		this.each(item => {
			if (val.nodeType === 1) {
	...
	// 克隆某个节点
	E.fn.cloneNode = function(){

	}
	// 添加和获取css方法 
	E.fn.css = function(val){
		if (!val) {
	...
	// 判断是否有某一个class
	E.fn.hasClass = function(className){
	...
	// 移除class
	E.fn.removeClass = function(className){
		this.each(item => {
			if (item.className.indexOf(className) !== -1) {
				let str, arr;
	...
	// 切换 class
	E.fn.toggleClass = function(className){
	...
	// 本地储存
	E.fn.getStorage = function(key){
		return localStorage.getItem(key);
	}
	E.fn.setStorage = function(key,value){
		localStorage.setItem(key,value)
	}
	E.fn.removeStorage = function(key){
